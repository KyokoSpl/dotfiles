...
myEditor :: String
myEditor = "Zed"  -- Sets emacs as editor

myVim :: String
myVim = myTerminal ++ " -e nvim "    -- Sets vim as editor
...
myStartupHook = do
  ...
  spawnOnce "bash ~/.screenlayout/test_vert_layout.sh"
...
myKeys c =
  ...
  , ("M-S-q", addName "Quit XMonad"            $ spawn "dm-logout -r")
  ...
  , ("M-S-,", addName "kb layout"              $ spawn "~/.config/xmonad/kblayout.sh")]
  ...
  [ ("M-p h", addName "List all dmscripts"     $ spawn "dm-hub -r")
  , ("M-p a", addName "Choose ambient sound"   $ spawn "dm-sounds -r")
  , ("M-p b", addName "Set background"         $ spawn "dm-setbg-r")
  , ("M-p c", addName "Choose color scheme"    $ spawn "~/.local/bin/dtos-colorscheme")
  , ("M-p C", addName "Pick color from scheme" $ spawn "dm-colpick -r")
  , ("M-p e", addName "Edit config files"      $ spawn "dm-confedit -r")
  , ("M-p i", addName "Take a screenshot"      $ spawn "dm-maim -r")
  , ("M-p k", addName "Kill processes"         $ spawn "dm-kill -r")
  , ("M-p m", addName "View manpages"          $ spawn "dm-man -r")
  , ("M-p n", addName "Store and copy notes"   $ spawn "dm-note -r")
  , ("M-p o", addName "Browser bookmarks"      $ spawn "dm-bookman -r")
  , ("M-p p", addName "Passmenu"               $ spawn "passmenu -p \"Pass: \"")
  , ("M-p q", addName "Logout Menu"            $ spawn "dm-logout -r")
  , ("M-p r", addName "Listen to online radio" $ spawn "dm-radio -r")
  , ("M-p s", addName "Search various engines" $ spawn "dm-websearch -r")
  , ("M-p t", addName "Translate text"         $ spawn "dm-translate -r")
  , ("M-p x", addName "logout menu"	       $ spawn "dm-logout -r")
  ]


  ^++^ subKeys "eww panels"
  [ ("M-S-e s", addName "open sidepanel"       $ spawn "eww open-many weather_side time_side smol_calendar player_side sys_side sliders_side | bash ~/.config/eww/scripts/getweather")
  , ("M-S-e d", addName "open Dashboard"       $ spawn "eww open-many blur_full weather profile quote search_full disturb-icon vpn-icon home_dir screenshot power_full reboot_full lock_full logout_full suspend_full | bash ~/.config/eww/scripts/getweather")
  , ("M-S-e c", addName "close all eww"        $ spawn "eww close-all")
  ]

  ^++^ subKeys "Favorite programs"
  [ ("M-<Return>", addName "Launch terminal"   $ spawn (myTerminal))
  , ("M-b", addName "Launch web browser"       $ spawn (myBrowser))
  , ("M-S-f", addName "Launch mFilemanager"    $ spawn (myFilemanager))
  ...
  ^++^ subKeys "Dev"
  [("M-e e", addName "Main Editor"               $ spawn (myEditor))
  -- ("M-e e", addName "Emacsclient Dashboard"    $ spawn (myEmacs ++ ("--eval '(dashboard-refresh-buffer)'")))
  , ("M-e v", addName "vsCode"                   $ spawn ("code"))
  , ("M-e a", addName "api Testing"              $ spawn ("postman"))
  , ("M-e n", addName "Neovim"                   $ spawn (myVim))
  , ("M-e p", addName "pycharm"                  $ spawn ("pycharm-community-eap"))
  , ("M-e r", addName "rust rover"               $ spawn ("rustrover-eap"))]
...
main = do
  ...
  xmproc2 <- spawnPipe ("xmobar -x 2 $HOME/.config/xmobar/" ++ colorScheme ++ "-xmobarvertrc")
